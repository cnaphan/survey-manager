	...
	<table id="question-${q.questionId}" class="matrix-table">
		<thead>
			<tr>
				...
				<g:each in="${ 1..q.attributes.matrixRange }" var="col" status="colNum">
					<th class="matrix-column">
						<g:if test="${ q.attributes.rangeLowText && (colNum == 0) }">
							<div class="matrix-range-text">${q.attributes.rangeLowText }</div>
						</g:if>
						<g:elseif test="${ q.attributes.rangeHighText && (colNum == (1..q.attributes.matrixRange).size() - 1) }">
							<div class="matrix-range-text">${q.attributes.rangeHighText }</div>
						</g:elseif>
						${col}
					</th>
				</g:each>
				<g:if test="${ q.attributes.hasNa }">
