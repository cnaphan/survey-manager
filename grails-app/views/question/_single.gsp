...
<div class="choice-group single" id="question-${q.questionId}">
	<g:if test="${q.control == Question.Control.SELECT }">
		...
		<div class="other-item">
			<label for="others.${q.questionId}">${q.hasOtherText ? q.hasOtherText : message(code:"default.hasOther.label") }</label>
	...
	<g:else test="${q.control == Question.Control.RADIO }">
		...
		<div class="other-item">
			...
			<label for="otherRadio.${q.questionId }" class='radio-item-label radio-other-item-label'>${q.hasOtherText ? q.hasOtherText : message(code:"default.hasOther.label") }</label> <g:textField name="others.${q.questionId}"  value="${a?.textValue }"/>
		</div>
		</g:if>				
	</g:else>
</div>	